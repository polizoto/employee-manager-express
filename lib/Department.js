...
const viewBudget = async () => {
  const getDepartments = await new Promise( ( resolve, reject ) => {
    ...
    const getDepartmentID = await new Promise( ( resolve, reject ) => {
     ...
     return new Promise( ( resolve, reject ) => {
      const sql = `SELECT sum(roles.salary) AS Total_Salary FROM employees LEFT JOIN roles ON employees.role_id = roles.id LEFT JOIN departments ON roles.department_id = departments.id WHERE departments.id = ?;`
      const params = [department_id];
    return db.query(sql, params, (err, rows) => {
      if (err) {
        reject (err)
...
const viewEmployees = async () => {
  const getDepartments = await new Promise( ( resolve, reject ) => {
    ...
    const getDepartmentID = await new Promise( ( resolve, reject ) => {
     ...
     return new Promise( ( resolve, reject ) => {
      const sql = `SELECT employees.id AS ID, concat(employees.first_name, ' ', employees.last_name) AS Employee, roles.role_name AS Role FROM employees LEFT JOIN roles ON employees.role_id = roles.id LEFT JOIN departments ON roles.department_id = departments.id WHERE departments.id = ?`
      const params = [department_id];
    return db.query(sql, params, (err, rows) => {
      if (err) {
        reject (err)
